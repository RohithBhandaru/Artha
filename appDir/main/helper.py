...
def monthlyTxnDatPl(local_month_year, email):
    return [
        {"$match": {"date": {"$gte": local_month_year,
                             "$lt": local_month_year + relativedelta(months=+1)}, "email": email}},
        {"$project": {"_id": "$_id", "type": "$category_type",
                      "name": "$category_name", "amount": "$amount"}},
        {"$group": {"_id": {"Type": "$type", "Name": "$name"},
                    "Amount": {"$sum": "$amount"}, "Number": {"$sum": 1}}},
...
def getMonthlyTxnData(local_month_year, cUser):
    data1 = list(mclient["artha"]["transaction_data"].aggregate(
        monthlyTxnDatPl(local_month_year, cUser.email)))
    data2 = list(mclient["artha"]["transaction_data"].aggregate(
        monthlyTxnDatSummaryPl(local_month_year + relativedelta(months=-1), cUser.email)))
    ...
    current_month_num["income-mom-percent"] = (
        (current_month_num["income"]/last_month_num["income"]) - 1)*100
    current_month_num["expense-mom-percent"] = (
        (current_month_num["expense"]/last_month_num["expense"]) - 1)*100
...
def getCategoriesList(catType, cUser):
    categoryPl = [
        {"$match": {"$and": [{"email": cUser.email},
                             {"category_type": catType}]}},
        {"$project": {"category_name": "$category_name"}}
    ]

    categories = list(
        mclient["artha"]["transaction_data"].aggregate(categoryPl))
...
def getCatHistoryPl(category, cUser):
    return [
        ...
        {"$group": {"_id": {"month": {"$month": {"date": "$date", "timezone": cUser.timezone}}, "year": {"$year": {
            "date": "$date", "timezone": cUser.timezone}}}, "totAmount": {"$sum": "$amount"}, "numTxn": {"$sum": 1}}},
        {"$sort": {"_id.year": 1, "_id.month": 1}}
    ]


def getCategoryHistoryData(category, cUser):
    dat = list(mclient["artha"]["transaction_data"].aggregate(
        getCatHistoryPl(category, cUser)))

    sDate = dt.date(dat[0]['_id']['year'], dat[0]['_id']
                    ['month'], 1).strftime('%Y-%m-%d')
    eDate = dt.date(dat[-1]['_id']['year'], dat[-1]['_id']
                    ['month'], 1).strftime('%Y-%m-%d')
    ...
    for ii in range(len(dat)):
        dfDat.loc[dt.datetime(dat[ii]['_id']['year'], dat[ii]['_id']['month'], 1)] = [
            dat[ii]['totAmount'], dat[ii]['numTxn']]
...
def getMFPortfolioPl(cUser):
    return [
        {"$match": {"$and": [{"email": cUser.email}, {
            "unit_balance": {"$ne": 0}}, {"events.amount": {"$ne": np.nan}}]}},
...
def getMFPortfolio(cUser, local_tz):
    funds = list(mclient["artha"]["mf_data"].aggregate(
        getMFPortfolioPl(cUser)))
    funds = pd.DataFrame([(f["fund_name"], f["folio_number"], f["as_on_date"], f["unit_balance"], f["nav"], f["total_invested"], UTC.localize(
        f["investing_since"]).astimezone(local_tz)) for f in funds], columns=['Fund Name', 'Folio Num', 'As On Date', 'CUB', 'NAV', 'Total Investment', 'Investing Since'])
    funds.loc[:, 'Fund Name'] = funds.loc[:,
                                          'Fund Name'].apply(lambda x: x.split('-')[1])
    funds['Total Value'] = funds['CUB']*funds['NAV']
    funds = funds.groupby(['Fund Name']).agg({'Folio Num': list, 'As On Date': max, 'CUB': 'sum',
                                              'NAV': max, 'Total Investment': 'sum', 'Total Value': 'sum', 'Investing Since': min}).reset_index()
    funds['P&L'] = funds['Total Value'] - funds['Total Investment']
    funds['P&L %'] = funds['P&L']*100/funds['Total Investment']
    funds = funds.sort_values(
        by=['P&L %'], ascending=False).reset_index(drop=True)
...
def getPaginatePl(cUser, skip, limit):
    return [
        {"$match": {"email": cUser.email}},
        {"$project": {"_id": "$_id", "date": "$date", "type": "$category_type",
                      "name": "$category_name", "description": "$description", "amount": "$amount"}},
        {"$sort": {"date": -1}},
        {"$skip": skip},
        {"$limit": limit}
    ]
...
def updateDailyTxnData(cUser):
    ...
    txnDat = pd.read_excel('appDir/user-content/' +
                           current_app.config["DAILY_TXN_FILE_NAME"], None)
    temp = pd.DataFrame()
...
def updateMFTxnData(cUser):
    ...
    df = read_pdf(pdfName, columns=[70.5, 333, 375, 433, 492], stream=True,
                  guess=False, pages='all', password=current_app.config["MF_STATEMENT_PWD"])
    ...
    for ii in folioData.keys():
        ...
        t1 = temp[temp.iloc[:, 0].str.contains(
            folioNumReg).replace(np.nan, False)]
