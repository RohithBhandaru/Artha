...
def summary():
    ...
    txn_E, txn_I, monthly_num = helper.getMonthlyTxnData(
        local_month_year, current_user)
...
def addNewTxn():
    ...
    if request.method == "POST":
        ...
        print("{0} - {1} - {2} - {3} - {4}".format(date,
                                                   amount, txn_type, category, description))
...
def updateData():
    if request.method == 'POST':
        ...
        if f1.filename != '':
            f1.save(os.path.join('appDir/user-content',
                                 current_app.config["DAILY_TXN_FILE_NAME"]))
            helper.updateDailyTxnData(current_user)
            flash("Transactions data updated!", category="success")
        if f2.filename != '':
            f2.save(os.path.join('appDir/user-content',
                                 current_app.config["MF_TXN_FILE_NAME"]))
            helper.updateMFTxnData(current_user)
            flash("Mutual funds data updated!", category="success")
        if f3.filename != '':
            f3.save(os.path.join('appDir/user-content',
                                 current_app.config["EQ_TXN_FILE_NAME"]))
...
def profile():
    ...
    txns = list(mclient["artha"]["transaction_data"].aggregate(
        helper.getPaginatePl(current_user, skip, limit)))
    for ii in txns:
        ii["date"] = UTC.localize(ii["date"]).astimezone(local_tz)

    try:
        list(mclient["artha"]["transaction_data"].aggregate(
            helper.getPaginatePl(current_user, skip+1, limit)))
...
def modifyDeleteTxn(txn_id):
    ...
    if request.method == "POST":
        ...
        if 'update' in request.form:
            ...
            mclient["artha"]["transaction_data"].delete_one(
                {"_id": ObjectId(txn_id)})
