...
def getMFPortfolio(cUser):
    ...
    funds = pd.DataFrame([(f["fund_name"], f["folio_number"], f["as_on_date"], f["unit_balance"], f["nav"], f["total_invested"], f["investing_since"])
                          for f in funds], columns=['Fund Name', 'Folio Num', 'As On Date', 'CUB', 'NAV', 'Total Investment', 'Investing Since'])
...
def getCatHistoryPl(category, cUser, local_tz_str):
    return [
        ...
        {"$group": {"_id": {"month": {"$month": {"date": "$date", "timezone": local_tz_str}}, "year": {"$year": {
            "date": "$date", "timezone": local_tz_str}}}, "totAmount": {"$sum": "$amount"}, "numTxn": {"$sum": 1}}},
...
def getCategoryHistoryData(category, cUser, local_tz_str):
    dat = list(mclient["artha"]["transaction_data"].aggregate(
        getCatHistoryPl(category, cUser, local_tz_str)))
